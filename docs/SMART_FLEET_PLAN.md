...
- **Document discrepancy detection** (`discrepancy_service.dart`, shipped as Phase 2b): when a staged ingestion item looks like a record the vessel already holds, the review card shows which existing record it matches and exactly which fields disagree (old → new). Matching is by certificate name (abbreviations included, so "ISSC" finds the full title), officer + certificate type, PR number falling back to item name, and defect title. A field the document doesn't state is never treated as a disagreement. Nothing is merged or overwritten — accepting still creates a new record, and the banner says so, so correcting the existing record stays a deliberate act in its own module.
